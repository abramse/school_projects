...
mydb <- dbConnect(RMySQL::MySQL(), 
                  dbname = 'final', 
                  host = 'final.cvt3jyj4wa6f.us-east-1.rds.amazonaws.com', 
                  port = 3306,
                  user = 'admin', 
                  password = 'summer2022')
...
#dbGetQuery(mydb, 'UPDATE unit_characteristics_1 SET No_Selection = REPLACE(No_Selection,"Yes","1")')
  ...
  output$property <- renderDT(
    data <- dbGetQuery(
      ...
      JOIN financing_1 AS f
             ...
             WHERE p.proj_st =', input$proj_st_id, 
        ...
        f.QOZF
             FROM state_information_1 AS s
                ...
                INNER JOIN financing_1 AS f ON s.property_id = f.property_id
                 WHERE p.proj_st =', input$map_st_id,
               'AND ', input$map_br, '>= 1 
